...
) : IQueries
{
  public async Task<T?> ReadById<T>(
    ...
    where T : class, ITrackableIdentifiableEntity
  {
  ...
  {
    if (!modelType.IsAssignableTo(typeof(ITrackableIdentifiableEntity)))
    {
      throw new InvalidOperationException(
        $"Type {modelType} is not assignable to {typeof(ITrackableIdentifiableEntity)}"
      );
    }
