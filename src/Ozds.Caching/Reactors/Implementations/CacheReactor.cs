...
public class CacheReactor(IServiceProvider serviceProvider)
  : Reactor<CacheEventArgs, ICacheSubscriber, CacheHandler>(serviceProvider) { }

public class CacheHandler(IServiceProvider serviceProvider)
  ...
  )
  {
    ...
    var policyContext = eventArgs is CreateCacheEventArgs createCacheEventArgs
      ? new CreateCacheEventPolicyContext(
        serviceProvider,
        cache,
        createCacheEventArgs
      )
      : new CacheEventPolicyContext(serviceProvider, cache, eventArgs);
    await Task.WhenAll(
      eventArgs.CacheConfiguration.Policies.Select(async policy =>
      {
        try
        {
