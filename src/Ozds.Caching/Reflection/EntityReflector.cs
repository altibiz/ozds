...
public class EntityReflector(ITypeQueries typeQueries)
{
  ...
  {
    return ResolveEntityKeyFromId(identifiable.GetType(), identifiable.Id);
  }

  public string ResolveEntityKeyFromComposite(ICompositeEntity composite)
  {
    return ResolveEntityKeyFromId(composite.GetType(), composite.Id);
  }

  public string ResolveEntityKeyFromJoin(IJoinEntity join)
  {
    return ResolveEntityKeyFromId(join.GetType(), join.Id);
  }

  public string ResolveEntityKeyFromId<T>(string id)
    where T : IEntity
  {
    return ResolveEntityKeyFromId(typeof(T), id);
  }

  public string ResolveEntityKeyFromId(Type entityType, string id)
  {
