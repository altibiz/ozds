...
) : IEmailSender
{
  public void Send(EmailMessage message)
  {
    ...
    mimeMessage.From.Add(new MailboxAddress(options.Value.From.Name, options.Value.From.Address));
  ...
  public async Task SendAsync(EmailMessage message)
  {
    ...
    await client.ConnectAsync(options.Value.Smtp.Host, options.Value.Smtp.Port, options.Value.Smtp.Ssl);
    await client.AuthenticateAsync(options.Value.Smtp.Username, options.Value.Smtp.Password);
  ...
  public void SendBulk(IEnumerable<EmailMessage> messages)
  {
    var mimeMessages = messages.Select(m =>
    {
      ...
      mimeMessage.Body = new BodyBuilder() { HtmlBody = m.Content, }.ToMessageBody();
