...
public record class CompositePhasicMeasure<T> : PhasicMeasure<T>
  where T : struct,
...
public record class TriPhasicMeasure<T>(T ValueL1, T ValueL2, T ValueL3)
  : PhasicMeasure<T>
  where T : struct,
    IComparisonOperators<T, T, bool>,
    IAdditionOperators<T, T, T>,
...
public record class SinglePhasicSumMeasure<T>(T Value) : PhasicMeasure<T>
  where T : struct,
    IComparisonOperators<T, T, bool>,
    IAdditionOperators<T, T, T>,
    ISubtractionOperators<T, T, T>,
    ...
    IDivisionOperators<T, T, T>
{
  ...
  {
    {
      return this switch
      {
        CompositePhasicMeasure<T> composite => composite
          .Measures.OrderBy(measure =>
            measure switch
            {
  ...
  public T PhaseAverage()
  {
    return this switch
    {
      ...
      InstantaneousPhaseMeasure<T> instantaneous =>
        instantaneous.Avg.PhaseAverage(),
  ...
  {
    {
      return this switch
      {
        ...
        InstantaneousPhaseMeasure<T> instantaneous =>
          instantaneous.Min.PhaseTrough(),
        CumulativePhasicMeasure<T> cumulative => cumulative
          .Diff()
          .PhaseTrough(),
        TriPhasicMeasure<T> tri => tri.ValueL1 < tri.ValueL2
          ? tri.ValueL1 < tri.ValueL3
      ...
      IDivisionOperators<TConverted, TConverted, TConverted>
  {
    return this switch
    {
      CompositePhasicMeasure<T> composite =>
        new CompositePhasicMeasure<TConverted>(
          composite
            .Measures.Select(measure =>
              measure.ConvertPrimitiveTo<TConverted>()
            )
            .ToList()
        ),
      InstantaneousPhaseMeasure<T> instantaneous =>
        new InstantaneousPhaseMeasure<TConverted>(
      ...
      CumulativePhasicMeasure<T> cumulative =>
        new CumulativePhasicMeasure<TConverted>(
      ...
      SinglePhasicSumMeasure<T> single =>
        new SinglePhasicSumMeasure<TConverted>(
  ...
  public PhasicMeasure<T> Add(PhasicMeasure<T> rhs)
  {
    ...
    {
      (CompositePhasicMeasure<T> compositeLhs, _) => compositeLhs.Zip(
        rhs,
        (lhs, rhs) => lhs.Add(rhs)
      ),
      (_, CompositePhasicMeasure<T> compositeRhs) => compositeRhs.Zip(
        this,
        (rhs, lhs) => lhs.Add(rhs)
      ),
      ...
      (CumulativePhasicMeasure<T> cumulativeLhs, _) => cumulativeLhs.Zip(
        rhs,
        (lhs, rhs) => lhs.Add(rhs)
      ),
      (_, CumulativePhasicMeasure<T> cumulativeRhs) => cumulativeRhs.Zip(
        this,
        (lhs, rhs) => lhs.Add(rhs)
      ),
      (TriPhasicMeasure<T> triLhs, TriPhasicMeasure<T> triRhs) =>
        new TriPhasicMeasure<T>(
          triLhs.ValueL1 + triRhs.ValueL1,
          triLhs.ValueL2 + triRhs.ValueL2,
          triLhs.ValueL3 + triRhs.ValueL3
        ),
      (
        SinglePhasicSumMeasure<T> singleLhs,
        SinglePhasicSumMeasure<T> singleRhs
