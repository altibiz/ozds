...
public record class CompositeTariffMeasure<T> : TariffMeasure<T>
  where T : struct,
    ...
    IDivisionOperators<T, T, T>
{
  ...
  {
    {
      return this switch
      {
        CompositeTariffMeasure<T> composite => new CompositeDuplexMeasure<T>(
          composite
  ...
  {
    {
      return this switch
      {
        CompositeTariffMeasure<T> composite => new BinaryTariffMeasure<T>(
      ...
      IDivisionOperators<TConverted, TConverted, TConverted>
  {
    return this switch
    {
      CompositeTariffMeasure<T> composite =>
        new CompositeTariffMeasure<TConverted>(
          composite
  ...
  public TariffMeasure<T> Add(TariffMeasure<T> rhs)
  {
    return (this, rhs) switch
    {
      ...
      (BinaryTariffMeasure<T> left, BinaryTariffMeasure<T> right) =>
        new BinaryTariffMeasure<T>(
          left.T1.Add(right.T1),
          left.T2.Add(right.T2)
        ),
      _ => Null,
    };
  }
