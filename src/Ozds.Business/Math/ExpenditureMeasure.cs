...
public record class UsageExpenditureMeasure<T>(TariffMeasure<T> TrueUsage)
...
public record class SupplyExpenditureMeasure<T>(TariffMeasure<T> TrueSupply)
  : ExpenditureMeasure<T>
  where T : struct,
    IComparisonOperators<T, T, bool>,
    IAdditionOperators<T, T, T>,
    ...
    IDivisionOperators<T, T, T>
{
      ...
      IDivisionOperators<TConverted, TConverted, TConverted>
  {
    return this switch
    {
      UsageExpenditureMeasure<T> usage =>
        new UsageExpenditureMeasure<TConverted>(
          usage.TrueUsage.ConvertPrimitiveTo<TConverted>()
        ),
      SupplyExpenditureMeasure<T> supply =>
        new SupplyExpenditureMeasure<TConverted>(
