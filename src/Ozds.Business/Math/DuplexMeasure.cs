...
public record class CompositeDuplexMeasure<T> : DuplexMeasure<T>
  where T : struct,
  ...
  )
  {
    return other switch
    {
      ...
      _ => new CompositeDuplexMeasure<T>(
        Measures.Select(measure => selector(measure, other)).ToList()
      ),
    };
  }
}
    ...
    IDivisionOperators<T, T, T>
{
  ...
  public PhasicMeasure<T> DuplexNet()
  {
    return this switch
    {
      CompositeDuplexMeasure<T> composite => new CompositePhasicMeasure<T>(
        composite
      ...
      ImportExportDuplexMeasure<T> importExport => importExport.Import.Subtract(
        importExport.Export
      ),
  ...
  {
    {
      return this switch
      {
        ...
        ImportExportDuplexMeasure<T> importExport => importExport.Import.Add(
          importExport.Export
        ),
        NetDuplexMeasure<T> net => net.TrueNet,
      ...
      IDivisionOperators<TConverted, TConverted, TConverted>
  {
    return this switch
    {
      CompositeDuplexMeasure<T> composite =>
        new CompositeDuplexMeasure<TConverted>(
          composite
      ...
      ImportExportDuplexMeasure<T> importExport =>
        new ImportExportDuplexMeasure<TConverted>(
  ...
  )
  {
    return this switch
    {
      ...
      ImportExportDuplexMeasure<T> importExport =>
        new ImportExportDuplexMeasure<T>(
  ...
  public DuplexMeasure<T> Add(DuplexMeasure<T> rhs)
  {
    ...
    {
      (CompositeDuplexMeasure<T> lhsComposite, _) => lhsComposite.Zip(
        rhs,
        (lhs, rhs) => lhs.Add(rhs)
      ),
      (_, CompositeDuplexMeasure<T> rhsComposite) => rhsComposite.Zip(
        this,
        (rhs, lhs) => lhs.Add(rhs)
      ),
      (
        ImportExportDuplexMeasure<T> lhsImportExport,
        ImportExportDuplexMeasure<T> rhsImportExport
      ) => new ImportExportDuplexMeasure<T>(
