...
public record class CompositeDuplexMeasure(List<DuplexMeasure> Measures) : DuplexMeasure
{
  public T FromMostAccurate<T>(Func<DuplexMeasure, T> selector, T @default) =>
    Measures.FirstOrDefault(measure => measure is ImportExportDuplexMeasure) is { } importExport
      ? selector(importExport)
      : Measures.FirstOrDefault(measure => measure is NetDuplexMeasure) is { } net
      ? selector(net)
      : Measures.FirstOrDefault(measure => measure is AnyDuplexMeasure) is { } any
      ? selector(any)
      : @default;

  public CompositeDuplexMeasure Select(Func<DuplexMeasure, DuplexMeasure> selector) =>
  ...
  public CompositeDuplexMeasure Zip(PhasicMeasure other,
    ...
    other switch
    {
      ...
      _ => new CompositeDuplexMeasure(Measures.Select(measure => selector(measure, other)).ToList())
    };
...
{
  public static CompositeDuplexMeasure ZipDuplex(this CompositePhasicMeasure lhs, DuplexMeasure rhs,
    Func<PhasicMeasure, DuplexMeasure, DuplexMeasure> selector) =>
    rhs switch
    {
      CompositeDuplexMeasure rhsComposite => new CompositeDuplexMeasure(lhs.Measures.Zip(rhsComposite.Measures, selector).ToList()),
      _ => new CompositeDuplexMeasure(lhs.Measures.Select(measure => selector(measure, rhs)).ToList())
    };
}
...
public abstract record class DuplexMeasure
{
  ...
  public PhasicMeasure DuplexNet
  {
    get
    {
      return this switch
      {
        CompositeDuplexMeasure composite => composite.FromMostAccurate(measure => measure.DuplexNet, PhasicMeasure.Null),
  ...
  public static DuplexMeasure operator +(DuplexMeasure lhs, float rhs)
  {
    return lhs switch
    {
      CompositeDuplexMeasure composite => composite.Select(measure => measure + rhs),
  ...
  public static DuplexMeasure operator +(float lhs, DuplexMeasure rhs)
  {
    return rhs switch
    {
      CompositeDuplexMeasure composite => composite.Select(measure => lhs + composite),
      ImportExportDuplexMeasure importExport => new ImportExportDuplexMeasure(
  ...
  public static DuplexMeasure operator +(DuplexMeasure lhs, PhasicMeasure rhs)
  {
    ...
    {
      (CompositeDuplexMeasure composite, _) => composite.Zip(rhs, (lhs, rhs) => lhs + rhs),
      (_, CompositePhasicMeasure composite) => composite.ZipDuplex(lhs, (rhs, lhs) => lhs + rhs),
      (ImportExportDuplexMeasure importExport, _) => new ImportExportDuplexMeasure(
        importExport.Import + rhs,
        importExport.Export + rhs
      ),
  ...
  public static DuplexMeasure operator +(PhasicMeasure lhs, DuplexMeasure rhs)
  {
    return (lhs, rhs) switch
    {
      (CompositePhasicMeasure composite, _) => composite.ZipDuplex(rhs, (lhs, rhs) => lhs + rhs),
      (_, CompositeDuplexMeasure composite) => composite.Zip(lhs, (rhs, lhs) => lhs + rhs),
      (_, ImportExportDuplexMeasure importExport) => new ImportExportDuplexMeasure(
        lhs + importExport.Import,
        lhs + importExport.Export
      ),
