...
public class AgnosticPushRequestMeasurementConverter
{
  ...
  public AgnosticPushRequestMeasurementConverter(IServiceProvider serviceProvider)
  {
  ...
  public JsonObject ToPushRequest(IMeasurement measurement)
  {
    return _serviceProvider
      ...
      .FirstOrDefault(converter => converter.CanConvert(measurement.MeterId))
      ?.ToPushRequest(measurement)
        ?? throw new InvalidOperationException($"No converter found for measurement {measurement.GetType()}.");
  }
  ...
  public IMeasurement ToMeasurement(JsonObject pushRequest, string meterId, DateTimeOffset timestamp)
  {
    return _serviceProvider
      ...
      ?.ToMeasurement(pushRequest, meterId, timestamp)
        ?? throw new InvalidOperationException($"No converter found for meter {meterId}.");
  }

  public TMeasurement ToMeasurement<TMeasurement>(JsonObject pushRequest, string meterId, DateTimeOffset timestamp)
