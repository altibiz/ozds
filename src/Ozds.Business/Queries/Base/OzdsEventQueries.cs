...
public class OzdsEventQueries
{
  ...
  public async Task<T?> ReadSingle<T>(string id) where T : class, IEvent
  {
    ...
    var item = await queryable.FirstOrDefaultAsync(x => (x as EventEntity)!.Id == id);
  ...
  ) where T : class, IEvent
  {
    ...
    var ordered = filtered.OrderByDescending(x => (x as EventEntity)!.Timestamp);
    var items = await filtered.Skip((pageNumber - 1) * pageCount).Take(pageCount).ToListAsync();
