...
public static class EntityModelTypeMapper
{
  public static IQueryable<object> GetDbSet(DbContext context, Type modelType)
  {
    ...
    var setMethod = typeof(DbContext).GetMethod("Set") ??
                    throw new InvalidOperationException(
                      "Set method not found in DbContext");
    var dbSet =
      setMethod.MakeGenericMethod(entityType).Invoke(context, null) as
        IQueryable<object>;
    return dbSet ??
           throw new InvalidOperationException(
             $"DbSet not found for entity type {entityType.FullName}");
  }
  ...
  public static object ToEntity(object model)
  {
    ...
    var toEntityMethod = modelConversionExtensions.GetMethod("ToEntity") ??
                         throw new InvalidOperationException(
                           $"ToEntity method not found in model conversion extensions type {modelConversionExtensions.FullName}");
  ...
  private static Type GetEntityType(Type modelType)
  {
    ...
    return entityType ??
           throw new InvalidOperationException(
             $"Entity type not found for model type {modelType.FullName}");
  }
  ...
  private static Type GetModelType(Type entityType)
  {
    ...
    var modelNamespace =
      typeof(EntityModelTypeMapper).Namespace?.Replace(".Abstractions",
        string.Empty);
    var modelFullName = $"{modelNamespace}.{modelTypeName}";
    var modelType =
      typeof(EntityModelTypeMapper).Assembly.GetType(modelFullName);
    return modelType ?? throw new InvalidOperationException(
      $"Model type not found for entity type {entityType.FullName}");
  }
  ...
  private static Type GetModelConversionExtensionsType(Type modelType)
  {
    ...
    var modelNamespace =
      typeof(EntityModelTypeMapper).Namespace?.Replace(".Abstractions",
        string.Empty);
    var modelFullName = $"{modelNamespace}.{modelTypeName}";
    var modelConversionExtensions =
      typeof(EntityModelTypeMapper).Assembly.GetType(modelFullName);
    return modelConversionExtensions ?? throw new InvalidOperationException(
      $"Model conversion extensions type not found for model type {modelType.FullName}");
  }
}
