...
public class IgnoreKeysOptions(DbContext dbContext)
{
  private readonly Lazy<HashSet<(string, Type?)>> keys = new(() =>
    dbContext.GetKeys().Select(x => (x.Name, x.DeclaringType)).ToHashSet()
  );
