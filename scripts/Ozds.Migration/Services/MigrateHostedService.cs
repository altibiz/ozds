...
) : BackgroundService
{
  ...
  private async Task MigrateDataAsync(CancellationToken stoppingToken)
  {
    await using var scope = serviceProvider.CreateAsyncScope();

    var migrationQueries =
      scope.ServiceProvider.GetRequiredService<DataMigrationQueries>();
    var pendingMigrations = await migrationQueries.ReadPendingMigrations(
      stoppingToken
    );
    ...
    var migrationMutations =
      scope.ServiceProvider.GetRequiredService<DataMigrationMutations>();
    await migrationMutations.MigrateAsync(stoppingToken);
  }

  private async Task MigrateMessagingAsync(CancellationToken stoppingToken)
  {
    await using var scope = serviceProvider.CreateAsyncScope();

    var migrationQueries =
      scope.ServiceProvider.GetRequiredService<MessagingMigrationQueries>();
    var pendingMigrations = await migrationQueries.ReadPendingMigrations(
      stoppingToken
    );
    logger.LogInformation(
    ...
    var migrationMutations =
      scope.ServiceProvider.GetRequiredService<MessagingMigrationMutations>();
    await migrationMutations.MigrateAsync(stoppingToken);
  }

  private async Task MigrateJobsAsync(CancellationToken stoppingToken)
  {
    await using var scope = serviceProvider.CreateAsyncScope();

    var migrationQueries =
      scope.ServiceProvider.GetRequiredService<JobsMigrationQueries>();
    var pendingMigrations = await migrationQueries.ReadPendingMigrations(
      stoppingToken
    );
    logger.LogInformation(
    ...
    var migrationMutations =
      scope.ServiceProvider.GetRequiredService<JobsMigrationMutations>();
    await migrationMutations.MigrateAsync(stoppingToken);
  }
}
