  ...
  public NetworkUserInvoiceStateMachine(IConfiguration configuration)
  {
    ...
    Initially(
      When(AcknowledgeNetworkUserInvoice)
        .Then(context => context.Saga.NetworkUserInvoiceId
          = context.Message.NetworkUserInvoiceId)
        .Send(
          new Uri(config["InitiateNetworkUserInvoice"]
    ...
    WhenEnter(
      ...
      x => x
        ...
        .IfElse(
          context => context.Saga.RegistrationId is not null,
          x => x
            .Send(
              new Uri(config["RegisterNetworkUserInvoice"]
                      ?? throw new InvalidOperationException(
                        "RegisterNetworkUserInvoice endpoint not found")),
              context =>
                new RegisterNetworkUserInvoice(
                  context.Saga.NetworkUserInvoiceId,
                  context.Saga.RegistrationId!))
            .TransitionTo(Registered),
          x => x
            .Send(
              new Uri(config["CancelNetworkUserInvoice"]
                      ?? throw new InvalidOperationException(
